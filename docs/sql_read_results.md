...
The 2024A cohort has the highest average pre-work score. This may indicate that they put in more effort and submitted more projects compared to the 2024B and 2025A cohorts.

```sql
SELECT cohort, AVG(prereq_score) FROM students GROUP BY cohort
```
...
Most of them are from Jordan, which shows higher performance compared to students from other countries like Egypt and Palestine.
...
Omar submitted only 2 out of 30 projects, so he has low performance (or a low grade). However, he belongs to the cohort that has the highest average. This could be explained by the fact that most of his classmates in the same cohort have high grades, which raises the overall average.  